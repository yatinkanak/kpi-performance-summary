...
async def _seed(session: AsyncSession) -> None:
    ...
    for fp, ps, pe, val in [
        ...
        session.add(Estimate(
            company_id=acme.id, kpi_id=kpi.id, period_start=ps, period_end=pe,
    ...
    for as_of, val in [(date(2025, 4, 30), 50.0), (date(2025, 5, 15), 90.0)]:
        session.add(Estimate(
            company_id=acme.id, kpi_id=kpi.id, period_start=date(2025, 4, 1),
            period_end=date(2025, 6, 30), fiscal_period="2025Q2", est_type=EstimateType.qtd,
            value=val, as_of=as_of, source="seed",
        ))
    session.add(Estimate(
        company_id=beta.id, kpi_id=kpi.id, period_start=date(2025, 1, 1),
        period_end=date(2025, 3, 31), fiscal_period="2025Q1",
        est_type=EstimateType.historical, value=300.0, as_of=None, source="seed",
    ))
    await session.flush()
