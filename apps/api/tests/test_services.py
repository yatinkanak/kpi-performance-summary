...
async def test_publish_qtd_requires_as_of(seeded):
    payload = schemas.PublishEstimateIn(
        ...
        fiscal_period="2025Q2", est_type="qtd", value=120.0, as_of=None,
    )
...
async def test_publish_historical_rejects_as_of(seeded):
    payload = schemas.PublishEstimateIn(
        ...
        fiscal_period="2025Q1", est_type="historical", value=210.0, as_of=date(2025, 3, 31),
    )
...
async def test_publish_appends_and_latest_wins(db_session):
    ...
    db_session.add(
        Estimate(
            ...
            period_start=date(2025, 1, 1), period_end=date(2025, 3, 31),
            fiscal_period="2025Q1", est_type=EstimateType.historical, value=100.0,
            as_of=None, published_at=datetime(2025, 1, 1, tzinfo=timezone.utc), source="seed",
        )
    )
    await db_session.flush()
    ...
    out = await svc.publish_estimate(
        ...
        schemas.PublishEstimateIn(
            period_start=date(2025, 1, 1), period_end=date(2025, 3, 31),
            fiscal_period="2025Q1", est_type="historical", value=999.0, as_of=None,
        ),
    )
