...
async def list_companies(
    svc: ServiceDep,
    search: str | None = Query(None, description="Match ticker or name"),
...
@router.get("/companies/{ticker}/kpis/{kpi_id}/series", response_model=schemas.KpiSeriesOut)
...
async def export_series(
    ...
    w = csv.writer(buf)
    w.writerow(
        ["fiscal_period", "period_start", "period_end", "est_type", "value", "unit", "as_of"]
    )
